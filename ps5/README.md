...
1. `*exp`: reads the word at the address denoted by `exp`. 
The address must be word-aligned.

2. `*exp1 = exp2`: places the value of `exp2` in memory at the
address denoted by `exp1` and returns the value of `exp2` as the result. 
The address must be word aligned.

3. `exp(exp1, ... ,expn)`: function names are now first-class values. 
For instance, you can assign a function name such as `fact` to a local variable `x` 
and then invoke the function via `x(n)`.

4. `malloc(n)`: allocate `n` bytes worth of storage, 
and return a (word-aligned) pointer to the storage.
...
Some example input and output files are the `samples/` directory.  The
files in that directory with names `_input.scish` are inputs, and the
corresponding `_output.cish` are example compiler outputs. We include
outputs from two different compilers, to help get a sense for some
possibilities.

To submit your work, you will upload the completed `scish_compile.ml` to
GradeScope when the autograder is released.

Recall that compiling a lambda expression should: 

* generate a new Cish function `f` that takes one parameter (the environment).
* produce as a value a closure which is a pair of the function name `f` and the current environment.

Compiling an application `(e1 e2)` should cause
...
`ps5_scish` and `ps5_cish` that can be used to test your compiler. The former
...
```bash
./ps5_scish test.scish | ./ps5_cish
```

to quickly get an evaluated answer. Feel free to change how this works (
by modifying `cish.ml` and `scish.ml`) if you want to test your compiler
differently. For the interested, you can copy in your cish-to-RISC-V compiler
from the previous assignment and compile from scish to RISC-V.

You can find a few more test cases in the `tests/` subdirectory. I highly
encourage you to write additional tests.
