...
let parse_file() =
  ...
  let _ = 
    if not (Array.length argv = 2 || Array.length argv = 3)
    then (prerr_string ("usage: " ^ argv.(0) ^ " [file-to-parse] [simple, optional]\n");
    exit 1) in
  let ch = open_in argv.(1) in
  let _ =
    if (Array.length argv = 3) then
      if (argv.(2) = "true") then
        (Printf.printf "Omitting callee-saved register handling\n";
         Cfg_ast.omit_callee_save := true)
      else
        ()
  in
