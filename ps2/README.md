...
I chose the latter, so some files remain unimplemented or are irrelevant.

The relevant files for this implementation are: `ast.ml`, `eval.ml`, `fishyacc.ml`, `lex.mll`, and `parse.mly`.

Use `make yacc` to compile. Use the `ps2yacc` executable to run the tests.
...
is behaving correctly. You can find test cases in the `tests/` subdirectory.
